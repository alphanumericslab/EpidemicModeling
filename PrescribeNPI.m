function PrescribeNPI(start_date_str, end_date_str, ip_file, costs_file, output_file)
...
min_cases = 1; % the minimum cases start date for processing each region/country
...
populations_file = "xprize-sample-data/populations.csv";
import_opt = detectImportOptions(populations_file);
import_opt.VariableNamingRule = 'preserve';
all_populations_data = readtable(populations_file, import_opt);%'ReadVariableNames', true, 'VariableNamingRule', 'preserve', 'TextType', 'string', 'detectImportOptions', 'true');


% Read Oxford time-series data
import_opt = detectImportOptions(ip_file);
import_opt.VariableNamingRule = 'preserve';
...
all_data = readtable(ip_file, import_opt);%'ReadVariableNames', true, 'VariableNamingRule', 'preserve', 'TextType', 'string', 'detectImportOptions', 'true');
...
for k = 3 : NumGeoLocations%index_ : index_%240 %122 : 125%225 %1 : NumGeoLocations
    ...
    I0 = max(min_cases, NewCasesSmoothed(1)); % initial number of cases
    ...
    [~, S_MINUS, S_PLUS, S_SMOOTH, P_MINUS, P_PLUS, P_SMOOTH, K_GAIN, innovations, rho] = NewCaseEKFEstimatorWithOptimalNPI(control_input, NewCasesSmoothedNormalized(:)', params, s_init, Ps_init, s_final, Ps_final, w_bar, v_bar, Q_w, R_v, beta, gamma, inv_monitor_len, 1);
    ...
    if(TWO_ROUND_TRAINING)
        ...
        [~, S_MINUS2, S_PLUS2, S_SMOOTH2, P_MINUS2, P_PLUS2, P_SMOOTH2, K_GAIN2, innovations2, rho2] = NewCaseEKFEstimatorWithOptimalNPI(control_input, NewCasesSmoothedNormalized(:)', params, s_init, Ps_init, s_final, Ps_final, w_bar, v_bar, Q_w, R_v, beta, gamma, inv_monitor_len, 1);
