...
for k = 205 : 206% 1: NumGeoLocations
...
%     InterventionPlansAug = [cumsum(InterventionPlansLagged1, 1), InterventionPlansLagged1, cumsum(InterventionPlansLagged3, 1), InterventionPlansLagged3, ones(size(InterventionPlans, 1), 1)];
    ...
    if(true)
        ...
        numFeatures = size(x_data_train_standardized, 2);
        numResponses = 1;
        numHiddenUnits = 200;
        ...
        net = trainNetwork(x_data_train_standardized', y_data_train', layers, options);
        ...
        LambdaHatLSTM = ([y_data_train ; y_pred']*sig_y + mu_y)';
        CumLambdaHatLSTM = cumsum(LambdaHatLSTM(counter > train_samples))';
        NewCasesEstimateLSTM = [NewCasesSmoothed(1 : train_samples) ; NewCasesSmoothed(train_samples)*exp(CumLambdaHatLSTM)];
        
        figure
        hold on
        plot([y_data_train ; y_pred']);
