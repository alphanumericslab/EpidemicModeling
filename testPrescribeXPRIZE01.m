...
num_days_before_opt_control = 25;
num_days_during_opt_control = 150;
...
for k = 1 : 50% length(TrainedModelParams) % the first row has the names
    ...
    [J0_zero_control, J1_zero_control] = NPICost(s_zero_control.*i_zero_control.*alpha_zero_control, u_zero_control, diag(npi_weights) * ones(NumNPI, size(u_zero_control, 2)));
    ...
    [J0_full_control, J1_full_control] = NPICost(s_full_control.*i_full_control.*alpha_full_control, u_full_control, diag(npi_weights) * ones(NumNPI, size(u_full_control, 2)));
    ...
    Q_w = (params.dt)^2*diag([10.0*i0, 10.0*i0, q_alpha, q_lambda, q_lambda, q_lambda].^2); % Process noise covariance matrix
    ...
    Ps_final(4, 4) = 1e-4; % zero costates
    Ps_final(5, 5) = 1e-4; % zero costates
    Ps_final(6, 6) = 1e-4; % zero costates
    R_v = var(5.0e3/N_population); % Observation noise variance
    
    num_pareto_front_points = 1000;
    human_npi_cost_factor = linspace(0, 1, num_pareto_front_points);
    ...
    for ll = 1 : length(human_npi_cost_factor)
        ...
        %         [J0_opt_control(ll), J1_opt_control(ll)] = NPICost(S_SMOOTH(1, num_days_before_opt_control + 1: end, ll).*S_SMOOTH(2, num_days_before_opt_control + 1: end, ll).*S_SMOOTH(3, num_days_before_opt_control + 1: end, ll), opt_control_input(:, num_days_before_opt_control + 1: end), diag(npi_weights) * ones(NumNPI, num_days_during_opt_control));
...
%         close all
        ...
        [J0_opt_control(ll), J1_opt_control(ll)] = NPICost(s_opt_control.*i_opt_control.*alpha_opt_control, u_opt_control, diag(npi_weights) * ones(NumNPI, size(u_opt_control, 2)));
        if(0)
            figure
            subplot(211);
    ...
    for scenario = 1 : num_random_input_monte_carlo_runs
        ...
        [J0(scenario), J1(scenario)] = NPICost(s.*i.*alpha, u, diag(npi_weights) * ones(NumNPI, size(u, 2)));
